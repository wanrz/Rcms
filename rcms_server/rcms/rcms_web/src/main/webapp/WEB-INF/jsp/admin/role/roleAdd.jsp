...
function add(){
	...
	$("#roleAddBtn").attr("disabled",true);
	var formData = new FormData($('form')[1]);
	formData.append("dataIds",dataId);
	 $.ajax({
	    	url:"<%=request.getContextPath()%>/admin/role/roleAdd.ajax",
	        type: 'POST',
	        data:formData,
...
function checkAll(menuCode,obj){
	var x = document.getElementsByClassName("attributeIds "+menuCode+"_code");
	var index = 0;
...
$(function() {
	$(".content").on('hide.bs.modal', function () {
		$("#roleAddBtn").attr("disabled",false);
	}); 
})
</script>
            ...
            <div class="box-body">
	              ...
	              <div class="form-group">
	              	<div class="col-md-12">
		                ...
		                <div class="col-sm-5">
		                	<textarea type="text" class="form-control" id="roleDesc" name="roleDesc" ></textarea>
		                </div>
	              	</div>
	              </div>
	            <!--   <div class="form-group">
	              		...
	              		<label class="checkbox" menuCode="${menuList.menuCode}">${menuList.menuName}</label>
	              			<div class="myClass">
		              			...
		              			<c:forEach items="${menuList.attrList}" var="action">
				              		<label class="checkbox">
									  <input type="checkbox" onclick="checkAll('${menuList.menuCode}',this)" class="attributeIds ${menuList.menuCode}_code" name="attributeIds" value="${action.attributeId}" >${action.title}
									</label>
		              			</c:forEach>
	              			</div>
	              		</div>
	              	...
	              	<div class="row" >&nbsp;</div>
	           		<div class="col-md-12" align="center">
						<c:forEach items="${dataList}" var="data" >
							<button type="button"  class="btn btn-default data-btn" onclick="changeColor('${data.dataId}')" dataId="${data.dataId}" id="${data.dataId}_id">${data.dataName}</button>
						</c:forEach>
					</div>
	              </div>
              </form>
              <div class="row">&nbsp;</div>
				<div class="row">
	              	<div class="col-md-12" align="center">
	              		<div class="col-md-12" align="center">
	              		<button type="button" id="roleAddBtn" class="btn btn-success btn-sm" onclick="add()">保存
						</button>
