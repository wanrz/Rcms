...
//消息提醒
$(function() {
	var status = '${rptWeeklyBo.status}';
	if (status == "0" || status == "3") {
		timeOpinion();
		$("#weekBeginDate").attr("readonly",false);
	}else{
		$("#weekBeginDate").attr("readonly",true);
	}
	$("#weekBeginDate").val(dateFormat("${rptWeeklyBo.weekBeginDate}",true));
	$("#weekEndDate").val(dateFormat("${rptWeeklyBo.weekEndDate}",true));
	displayNone();
})
function displayNone() {
	var status = "${rptWeeklyBo.status}";
	var flag = "${rptWeeklyBo.projStatus}";
	if (status == "1" || status == "2" || flag == "结项") {
		divInit();
...
function unMoreNextMonday() {
	var weekBeginDate = $("#weekBeginDate").val();
	var date = new Date(weekBeginDate);
	var nowDate = new Date(dateFormat(new Date(),false));
	...
	var nowFriday = new Date(nowMonday.getTime() + (5*(1000 * 60 * 60 * 24)));
	if (nextMonday.getTime() <= date.getTime()){
		$("#weekReportWarning").text("周报开始日期不能大于本周末");
		$("#weekBeginDate").css("border-color","#dd4b39");
		return false;
	}
	if (nowMonday.getTime() < date.getTime() && date.getTime() < nowFriday.getTime()) {
...
function warning(boole,warn) {
	if (boole){
		$("#weekReportWarning").text(warn);
		$("#weekBeginDate").css("border-color","#dd4b39");
		return false;
	}
	$("#weekReportWarning").text("");
...
function weekReportSave() {
	$("#status").val("0");
	var url = "<%=request.getContextPath()%>/admin/weeklyReport/updateWeeklyReportSave.ajax";
	weekReportAjax(url);
}
function weekReportSubmit() {
	$("#status").val("1");
	var url = "<%=request.getContextPath()%>/admin/weeklyReport/updateWeeklyReportSubmit.ajax";
	weekReportAjax(url);
}
                ...
                <div class="row">
					<form action="" id="updateWeekReportForm" class="form-horizontal">
						<div class="form-group" >
								 <label class="col-sm-2 control-label" for="proRptId">周报周期<span>&nbsp;&nbsp;&nbsp;</span></label>
					             <input type="text"  name="status" hidden="hidden" value="${rptWeeklyBo.status}"/>
					             <div class="col-sm-2">
						...
						<div class="form-group formGroupTop" >
							 <label class="col-sm-2 control-label" for="milePost">里程碑<span id="star" class="star">&nbsp;&nbsp;*</span></label>
				             <div class="col-sm-8">
						...
						<!-- list -->
						<div class="form-group formGroupTop" >
							 <label class="col-sm-2 control-label" for="curRpt">本周事项汇报<span id="star" class="star">&nbsp;&nbsp;*</span></label>
				             <div class="col-sm-8">
						...
						<div class="form-group formGroupTop">
							 <label class="col-sm-2 control-label" for="nextRpt">下周重要事项<span id="star" class="star">&nbsp;&nbsp;*</span></label>
				             <div class="col-md-6" >
								<div id="nextRptTextarea"></div>
							 </div>
						</div>
						<div class="form-group formGroupTop">
							 <label class="col-sm-2 control-label" for="approvalOpinions">审批意见<span>&nbsp;&nbsp;&nbsp;</span></label>
				             <div class="col-md-6" >
								<div class="rptWeeklyBoDiv" id="approvalOpinion">${rptWeeklyBo.approvalOpinions}</div>
							 </div>
						</div>
					</div>
						<div class="row">&nbsp;</div>
						<div class="form-group">
				             ...
				             <div class="col-sm-4" align="center">
				             	<button type="button" class="btn btn-primary btn-sm" id="save" onclick="weekReportSave()">暂存</button>
				             	<button type="button" class="btn btn-success btn-sm" id="submit"  onclick="weekReportSubmit()">保存并提交</button>
				             	<button type="button" class="btn btn-danger btn-sm" id="cancel" onclick="back()">取消</button>
				             	<button type="button" class="btn btn-danger btn-sm"  id="backbtn" onclick="back()">返回</button>
				             </div>
						</div>
					</form>
				</div>
	            <div class="row">
	            	<div class="col-md-12">
	            		<form action="" id="weekReportForm">
					        ...
					        <input type="text"  id="pkProj" name="pkProj" hidden="hidden" value="${rptWeeklyBo.pkProj}"/>
...
<script type="text/javascript">
var milePostEditor,curRptEditor,curRiskEditor,nextRptEditor;
function editorInit() {
	milePostEditor = ueEditor("milePostTextarea");
	curRptEditor = ueEditor("curRptTextarea");
	curRiskEditor = ueEditor("curRiskTextarea");
	nextRptEditor = ueEditor("nextRptTextarea");
	...
	milePostEditor.addListener("ready", function () {
		milePostEditor.setContent('${rptWeeklyBo.milePost}');
		curRptEditor.setContent('${rptWeeklyBo.curRpt}');
		curRiskEditor.setContent('${rptWeeklyBo.curRisk}');
		nextRptEditor.setContent('${rptWeeklyBo.nextRpt}');
	});
	$(".star").html("&nbsp;&nbsp;*");
}

function divInit() {
	$("#milePostTextarea").html('${rptWeeklyBo.milePost}');
	$("#curRptTextarea").html('${rptWeeklyBo.curRpt}');
	$("#curRiskTextarea").html('${rptWeeklyBo.curRisk}');
	$("#nextRptTextarea").html('${rptWeeklyBo.nextRpt}');
	$("#milePostTextarea").addClass("rptWeeklyBoDiv");
	$("#curRptTextarea").addClass("rptWeeklyBoDiv");
	$("#curRiskTextarea").addClass("rptWeeklyBoDiv");
	$("#nextRptTextarea").addClass("rptWeeklyBoDiv");
	$(".star").html("&nbsp;&nbsp;&nbsp;");
}
</script>
</html>