...
$(function(){
	...
	$("#myTable").createTable({
		...
		url:"<%=request.getContextPath()%>/admin/change/projectQuery.ajax",//请求数据地址（必须）
...
function projectChange(){
	...
	if(proj.length != 1){
		window.top.warning("只能选择一条项目进行变更");
		return;
	}
	if(proj[0].status == "结项"){
		window.top.warning("该项目已经结项，不能进行变更");
		return;
	}
	$("#pkProj").val(proj[0].pkProj);
	$("#queryForm").attr("action","<%=request.getContextPath()%>/admin/change/projectUpdate.do")
	$("#queryForm").submit();
}
...
function projectDetails(){
	...
	$.ajax({
		type:"post",
		url:"<%=request.getContextPath()%>/admin/change/queryCount.ajax",
		data:{pkProj:proj[0].pkProj},
		success:function(data){
			var count = data.RSP_BODY.Data.count;
			if(count == 0){
				window.top.warning("该项目没有变更记录");
				return;
			}
			$("#pkProj").val(proj[0].pkProj);
			$("#queryForm").attr("action","<%=request.getContextPath()%>/admin/change/projectDetails.do")
			$("#queryForm").submit();
		}
	});
}
            ...
            <div class="box-body">
				...
				<div class="row">
	              	<div style="padding-left: 16px">
	              		<button type="button" class="btn btn-primary btn-sm" onclick="projectChange()">
						  <span class="glyphicon glyphicon-pencil" aria-hidden="true"><span class="my-operation">项目变更</span></span>
						</button>
						<button type="button" class="btn btn-primary btn-sm" onclick="projectDetails()">
						  <span class="glyphicon glyphicon-search" aria-hidden="true"><span class="my-operation">变更详情</span></span>
						</button>
	              	</div>
                </div>
	             <div class="row">
